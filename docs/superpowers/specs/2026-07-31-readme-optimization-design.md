...
## 1. 目标
优化根目录 `README.md`，增加最新引入的 `scanned-pdf-to-excel` 技能说明，使用清晰的结构与模块化表格展现技能矩阵、目录树、依赖安装以及使用示例。

## 2. README 结构设计

1. **项目标题与简介**：标明仓库为 Claude Code 自定义技能库。
2. **技能速查矩阵 (Skill Matrix)**：
   - 使用 Markdown 表格呈现每个技能的名称、适用场景、亮点/Token 优化策略及核心 Python 依赖。
3. **完整目录结构树**：
   - 覆盖 `pdf-to-gtp` 和 `scanned-pdf-to-excel` 两个技能组。
4. **安装与引入说明**：
   - 说明如何软链接或复制到 `~/.claude/skills/`。
5. **技能详细指南（模块化卡片）**：
   - 分别给出 `pdf-to-gtp` 和 `scanned-pdf-to-excel` 的依赖安装命令和主要脚本执行示例。
6. **贡献指南**。
